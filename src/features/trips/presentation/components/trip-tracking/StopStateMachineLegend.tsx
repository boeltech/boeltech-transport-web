...
export function StopStateMachineLegend({ className }: { className?: string }) {
  const { collapsed, expand, collapse } = useTrackingLegendCollapsed("stop");
  const copy = legendCopy.stop;
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-1.5 text-muted-foreground">
        ...
        <span className="text-muted-foreground/70">· alterna</span>
        <span className="rounded-full border border-dashed bg-background px-2 py-0.5 font-medium">
          {copy.alternate}
        </span>
      </div>
    </div>
