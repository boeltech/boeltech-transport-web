 ...
 * Frame + route-mesh: dirección «Industrial confiable» / Editorial logística.
 * `backdrop={false}` cuando el hero ya aporta capas (LandingHeroVisual).
 */
...
type LandingProductPreviewProps = {
  className?: string;
  /** Route-mesh detrás del frame. Default true (standalone). */
  backdrop?: boolean;
};

const PRODUCT_PREVIEW_ID = "product-preview-dashboard" as const;
const ROUTE_MESH_SRC = "/brand/tlama-landing-route-mesh.svg";
...
export function LandingProductPreview({
  ...
  if (isCommercialAssetEnabled(PRODUCT_PREVIEW_ID)) {
    return (
      <PreviewShell className={className} backdrop={backdrop}>
        <CommercialImage
          id={PRODUCT_PREVIEW_ID}
          decorative
          loading="eager"
          className="bg-card border-border w-full rounded-xl border object-cover object-top"
        />
      </PreviewShell>
    );
  }
    ...
    >
      {backdrop ? (
        <img
          ...
          height={420}
          decoding="async"
          className="pointer-events-none absolute -top-6 left-1/2 z-0 w-[115%] max-w-none -translate-x-1/2 opacity-70 sm:-top-10 sm:w-[108%]"
        />
      ) : null}
      <div className="landing-preview-frame relative z-10 overflow-hidden rounded-xl">
        {children}
      </div>
    </div>
  );
...
function LandingProductPreviewMock() {
  const { preview } = landingCopy;

  return (
    <div className="bg-card text-card-foreground border-border overflow-hidden rounded-xl border">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-3 p-3 sm:p-4">
          <div className="grid grid-cols-2 gap-2">
            {preview.statusStrip.map((item) => (
              <div
                key={item.label}
                className="bg-muted/50 rounded-lg border px-3 py-2.5"
              >
          ...
          <div className="bg-muted/30 min-h-0 flex-1 rounded-lg border p-3">
            ...
            <ul className="space-y-2">
              {preview.trips.map((trip) => (
                <li
                  key={trip.code}
                  className="bg-background/90 flex items-center justify-between gap-3 rounded-md border px-2.5 py-2"
                >
                  ...
                  <span className="bg-primary/15 text-primary shrink-0 rounded px-2 py-0.5 text-[10px] font-medium sm:text-xs">
                    {trip.status}
                  </span>
                </li>
              ))}
